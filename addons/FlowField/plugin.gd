...
func _draw_highlight(viewport: Control, tileSize : Vector2) -> void:
	...
	for tile_pos : Vector2i in _highlighted_tiles.keys():
		...
		#var draw_str : String = str(_highlighted_tiles[tile_pos].bias);
		...
		#
		#viewport.draw_set_transform(xform.get_origin(), 0.0, xform.get_scale() * scale);
		#viewport.draw_string(_default_font, ((tile_rect.position + (tileSize * 0.5)) / scale) + Vector2(-draw_size.x * 0.5, draw_size.y * 0.25), draw_str, HORIZONTAL_ALIGNMENT_RIGHT, -1, 32);
	#viewport.draw_set_transform_matrix(xform);
			...
			for y in rect_size.y:
				for x in rect_size.x:
					...
					if !_highlighted_tiles.has(look_up_pos):
						_highlighted_tiles[look_up_pos] = FlowFieldTile.new().set_bias(_set_bias);
	...
	if delta.x > delta.y:
		...
		while current.x != p_end.x:
			if !_highlighted_tiles.has(current):
				_highlighted_tiles[current] = FlowFieldTile.new().set_bias(_set_bias);
